...
if (count($errors) == 0) {
	...
	if ($idx) {
		...
		if ($post->midx != $this->IM->getModule('member')->getLogged()) {
			//$this->IM->getModule('push')->sendPush($post->midx,$this->getModule()->getName(),'answer',$idx,'answer_modify',array('idx'=>$idx,'from'=>$this->IM->getModule('member')->getLogged(),'title'=>$question->title));
		}
